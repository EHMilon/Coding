...
class UpdateListener extends dispose_1.Disposable {
  constructor(_userDataDir) {
    ...
    const notUserDataDirChange = function (file) {
      return (
        file.scheme != constants_1.UriSchemes.vscode_userdata &&
        (!_userDataDir ||
          !pathUtil_1.PathUtil.PathBeginsWith(file.fsPath, _userDataDir))
      );
    };
    this._debounceDelay =
  ...
  get _reloadOnAnyChange() {
    return (
      settingsUtil_1.SettingUtil.GetConfig().autoRefreshPreview ==
  ...
  get _reloadOnSave() {
    return (
      settingsUtil_1.SettingUtil.GetConfig().autoRefreshPreview ==
      settingsUtil_1.AutoRefreshPreview.onSave
    );
  }
  /**
